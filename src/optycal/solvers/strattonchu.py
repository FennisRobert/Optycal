...
#
# NOTE: the near-field surface->points kernel that used to live in this
# file (`stratton_chu_xyz`) has been removed -- `Surface.expose_xyz` now
# calls the Rust port (`optycal_kernels.stratton_chu_xyz`) by default. See
# claude_nodes/kernel_optimization.md for the full port history/benchmarks
# (which still document the numba numbers this file used to produce) and
# `tests/test_stratton_chu_dipole_sphere.py` for the ongoing,
# numba-independent correctness gate (the closed-sphere analytic-dipole
# equivalence test). `stratton_chu_xyz_surface` below (surface->surface,
# with Fresnel R/T per source point) has *not* been ported and remains
# numba -- `Surface.expose_surface` still calls it directly.

import numpy as np
from numba import c16, f8, njit, prange
